...
// slot (tap to reassign), bench order (move up/down), and per-player minutes
// (edit; totals stay locked at 240 via setMinutes). Auto button restores the
// computed lineup. Player rows show ovr/pot/age/contract and injury status.
// =============================================================================
...
function pickStarter(slot) {
  ...
  const rows = roster.map((p) => [
    `${p.name}`, p.pos, p.ovr, isHealthy(p) ? 'OK' : 'INJ',
  ]);
...
function dropPlayerMenu() {
  ...
  const rows = roster.map((p) => [p.name, p.pos, p.ovr, money(p.contract ? p.contract.amount : 0)]);
...
registerScreen('roster', {
  render() {
    ...
    const starterRows = SLOTS.map((slot) => {
      ...
      return [slot, p ? p.name : '—', p ? p.ovr : '', p ? (L.minutes[p.pid] || 0) : ''];
    });
    ...
    L.bench.forEach((pid) => {
      ...
      benchWrap.append(panel(
        el('div', { class: 'row' },
          el('span', { text: `${p.name} · ${p.pos} · ovr ${p.ovr}${isHealthy(p) ? '' : ' · INJ'}` }),
